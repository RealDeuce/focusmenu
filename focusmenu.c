...
static struct config *config_tail = NULL;

static char *get_window_name(Window w);
static char *focused_window_name(void);
static int open_display(void);

static Display *disp;
static Window last_window;
static bool got_window;
...
static char *get_window_name(Window w)
{
	...
	if (XGetWMName(disp, w, &prop)) {
		char **names = NULL;
		int cnt = 0;

		if (XmbTextPropertyToTextList(disp, &prop, &names, &cnt) == Success) {
			ret = strdup(names[0]);
...
static char *focused_window_name(void)
{
	...
	XGetInputFocus(disp, &win, &ignored);
	if (got_window && last_window == win)
		return NULL;
	got_window = true;
	last_window = win;
	parent = win;
	while (parent != root) {
		win = parent;
		if (XQueryTree(disp, win, &root, &parent, &children, &cnt))
			XFree(children);
	}
	parent = win;
...
static void pregerror(size_t lno, int err, const regex_t *preg)
{
	size_t newlen;
	char *toa, *msg;

	newlen = regerror(err | REG_ITOA, preg, NULL, 0);
	toa = malloc(newlen);
	if (toa == NULL) {
		fprintf(stderr, "Error %d compiling regex line %zu\n", err, lno);
	}
	else {
		regerror(err | REG_ITOA, preg, toa, newlen);
		newlen = regerror(err, preg, NULL, 0);
		msg = malloc(newlen);
		if (msg == NULL) {
			fprintf(stderr, "Error %s (%d) compiling regex line %zu\n", toa, err, lno);
		}
		else {
			regerror(err, preg, msg, newlen);
			fprintf(stderr, "Error %s (%d) compiling regex line %zu: %s", toa, err, lno, msg);
			free(msg);
		}
		free(toa);
	}
...
static void parse_config(void)
{
	...
	while ((linelen = getline(&line, &linesz, f)) >= 0) {
		lno++;
		// Skip lines where the first non-whitespace character is a #
		for (p = line; isspace(*p) && p; p++)
			;
		if (*p == '#')
			continue;
		if (line[0] == '\t') {	// Tab introduces a text line
			if (ent == NULL) {
				fprintf(stderr, "Config error line %zu: string doesn't follow regex\n", lno);
				continue;
			}
			newlen = ent->text_len + linelen - 1;
			p = realloc(ent->text, newlen);
			if (p == NULL) {
				fprintf(stderr, "Memory error line %zu: Cannot allocate space for line.  Line ignored\n", lno);
				continue;
			}
			ent->text = (unsigned char *)p;
			memcpy(&ent->text[ent->text_len], &line[1], linelen - 1);
			ent->text_len = newlen;
		}
		else {			// regexp starts at beginning of line
			if (line[0] == 0 || line[0] == '\n')
				continue;
			ent = calloc(1, sizeof(*ent));
			if (ent == NULL) {
				fprintf(stderr, "Memory error line %zu: Cannot allocate space for entry.  Entry will be ignored.\n", lno);
				continue;
			}
			// Strip newline...
			p = strrchr(line, '\n');
			if (p)
				*p = 0;
			rval = regcomp(&ent->match, line, REG_EXTENDED | REG_NOSUB);
			if (rval != 0) {
				pregerror(lno, rval, &ent->match);
				regfree(&ent->match);
				free(ent);
				ent = NULL;
			}
...
static void draw_glyph(unsigned char ch, unsigned int *x, unsigned int *y)
{
	// TODO: No way to get display extents...
	const unsigned char *p = &font[ch * 16];
	int yoff, xoff;
	...
	for (yoff = 0; yoff < 16; yoff++) {
		for (xoff = 0; xoff < 8; xoff++) {
			if (*x + xoff < 240 && *y + yoff < 64) {
				if (*p & (1 << xoff))
					tdx_pixel(*x + (7 - xoff), *y + yoff, 0);
			}
		}
		p++;
	}
...
static void display_entry(const unsigned char *ent, size_t len)
{
	size_t i;
	unsigned x = 0, y = 0;
	...
	tdx_clear(0xff);
	for (i = 0; i < len - 1; i++) {
		draw_glyph(ent[i], &x, &y);
	}
	tdx_update();
}

static void match_title(const char *title)
{
	struct config *c;

	for (c = config; c; c = c->next) {
		if (regexec(&c->match, title, 0, NULL, 0) == 0) {
			display_entry(c->text, c->text_len);
			return;
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	if (argc > 1) {
		...
		configpath = "/usr/local/etc/focusmenu";
	}
	parse_config();
	...
	atexit(cleanup);
	signal(SIGHUP, do_terminate);
	signal(SIGINT, do_terminate);
	signal(SIGKILL, do_terminate);
	signal(SIGTERM, do_terminate);
	signal(SIGXCPU, do_terminate);
	signal(SIGXFSZ, do_terminate);
	signal(SIGUSR1, do_terminate);
	signal(SIGUSR2, do_terminate);
	signal(SIGTHR, do_terminate);
	signal(SIGTHR, do_terminate);

	ts.tv_sec = 0;
	ts.tv_nsec = 500000000;
	while (!terminate) {
		name = focused_window_name();
		if (name) {
			match_title(name);
		}
		free(name);
